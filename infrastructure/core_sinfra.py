...
class CoreSaidaInfrastructure(Stack):
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        hosted_zone = PublicHostedZone(self, f"{construct_id}PublicHostedZone", subdomain="team-saida")

        # This code fixes an autogenerated policy name for the
        # `CrossAccountZoneDelegationRecord` which would be created with more than 128
        # chars. At some point this code is going to live in `bhub_cdk` but, for now it
        # makes the deploy work on production accounts.
        for child in hosted_zone.node.children:
            if not isinstance(child, route53.CrossAccountZoneDelegationRecord):
                continue

            for grand_child in child.node.children:
                if not isinstance(grand_child, cdk.CustomResource):
                    continue

                for dependency in grand_child.node.dependencies:
                    if not isinstance(dependency, iam.Policy):
                        continue

                    cfn_policy = dependency.node.default_child
                    cfn_policy.add_property_override(
                        "PolicyName",
                        f"{hosted_zone.node.id}CrossAccountZoneDelegationRecordHandlerRolePolicy",  # noqa: E501
                    )
